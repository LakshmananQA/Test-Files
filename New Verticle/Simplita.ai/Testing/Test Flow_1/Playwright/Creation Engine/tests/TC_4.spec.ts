...
test.describe('Verify Creation Engine functionality', () => {
    ...
    // Test: Edit the Employee Name field in the form preview
    test('Verify renaming Employee Name label and updating field value', async ({ page }) => {
        try {
            // Locate the "Employee Name" field label
            const employeeNameLabel = page.locator('input[value="Employee Name"]');



            // Click on the "Employee Name" field label
            await employeeNameLabel.click();

         
            await employeeNameLabel.fill('Employee Code');

            const employeeEmailLabel = page.locator('input[value="Employee Email"]');
            await employeeEmailLabel.click();
            
                     // Verify the "Employee Name" label is renamed to "Employee Code"
                     const employeeNameLabel1 = page.locator('input[value="Employee Code"]');
            await expect(employeeNameLabel1).toBeVisible();
        } catch (error) {
